...
class JMXItemChecker extends ItemChecker
{
	...
	public JSONArray getValues() throws ZabbixException
	{
		...
		try
		{
			...
			{
				logger.debug("put credentials!! {}:{}", username, password);
				env = new HashMap();
	...
	protected String getStringValue(String key) throws Exception
	{
		...
		if (item.getKeyId().equals("jmx"))
		{
			...
			logger.debug("jmx ObjectName domain : " + objectName.getDomain());
			if(objectName.getDomain().startsWith("com.bea")){
				return getPrimitiveAttributeValue(wlMbsc.getAttribute(objectName, realAttributeName), fieldNames);
			}
	...
	private boolean isWeblogicKey(){
		boolean isWLKey = false;
		for (String key : keys){
			if(key.indexOf("com.bea:") > -1){
				isWLKey = true;
				break;
			}
		}
	...
	private boolean hasOtherKey(){
		boolean isOTKey = false;
		for (String key : keys){
			if(key.indexOf("com.bea:") == -1){
				isOTKey = true;
				break;
